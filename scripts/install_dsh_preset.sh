...
MANAGED_MARKER="${PRESET_TARGET}/.managed-by-ai-pm-model-harness"

if [[ -e "${PRESET_TARGET}" && ! -f "${MANAGED_MARKER}" ]]; then
...
cp "${PRESET_SOURCE}/.managed-by-ai-pm-model-harness" "${MANAGED_MARKER}"
